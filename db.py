...
class QfpDB:
    ...
    def _store_peaks(self, c, fp, recordid):
        ...
        """
        for x, y in fp.peaks:
            #print("(x,y) in peaks:",(x,y))
            c.execute("""INSERT INTO Peaks VALUES (?,?,?)""", (recordid, x, y))
    ...
    def query(self, fp, vThreshold=0.005):
        """
    ...
    def _radius_nn(self, c, h, e=0.01):
        ...
        """
        c.execute('SELECT id FROM Hashes WHERE minW >= ? AND maxW <= ? AND minX >= ? AND maxX <= ? AND minY >= ? AND maxY <= ? AND minZ >= ? AND maxZ <= ?',
                  (h[0] - e,       h[0] + e,
                   h[1] - e,       h[1] + e,
                   h[2] - e,       h[2] + e,
                   h[3] - e,       h[3] + e))
    ...
    def _filter_candidates(self, conn, c, qQuad, results, e=0.2, eFine=1.8):
        ...
        for hashid in c:
            ...
            #print("qQuad:",qQuad.A.y )
            #print("cQuad:",list(cQuad.A.y)[0])
            #if not 1 / (1 + e) <= qQuad.A.y / list(cQuad.A.y)[0] <= 1 / (1 - e):
            #    continue
            # X transformation tolerance check:
            #   sTime = (queBx-queAx)/(canBx-canAx)
            sTime = (qQuad.B.x - qQuad.A.x) / (list(cQuad.B.x)[0] - list(cQuad.A.x)[0])
            #print("sTime:",sTime)
            #if not 1 / (1 + e) <= sTime <= 1 / (1 - e):
            #    continue
            # Y transformation tolerance check:
            #   sFreq = (queBy-queAy)/(canBy-canAy)
            sFreq = (qQuad.B.y - qQuad.A.y) / (list(cQuad.B.y)[0] - list(cQuad.A.y)[0])
            ...
            #if not abs(qQuad.A.y - (list(cQuad.A.y)[0] * sFreq)) <= eFine:
            #    continue
            offset = list(cQuad.A.x)[0] - (qQuad.A.x / sTime)
            #print("offset:",offset)
            #print("(offset, (sTime, sFreq):",(offset, (sTime, sFreq)))
    ...
    def _lookup_peak_range(self, c, recordid, offset, e=3750):
        ...
        #print("(offset, offset + e, recordid):",(offset, offset + e, recordid))
        c.execute("""SELECT X, Y, recordid
                       FROM Peaks""")
                      #WHERE X >= ? AND X <= ?
                       # AND recordid = ?""", data)
        essential = []
        for X,Y, recordid1 in c.fetchall():
          if float(list(X)[0]) >= offset or float(list(X)[0]) <= offset + e:
             if int(recordid1) == int(recordid):
                essential.append((list(X)[0],list(Y)[0],recordid1))
        #print("essential:", essential)
        return [self.Peak(p[0], p[1]) for p in essential]
    ...
    def _verify_peaks(self, mc, rPeaks, qPeaks, eX=18, eY=12):
        ...
        for rPeak in rPeaks:
            ...
            #print("rPeakScaled:",rPeakScaled)
            lBound = bisect_left([i[0] for i in qPeaks], (rPeakScaled.x - eX))
            rBound = bisect_right([i[0] for i in qPeaks], (rPeakScaled.x + eX))
